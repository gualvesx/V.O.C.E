...
app.post('/api/classes/:classId/edit', requireLogin, async (req, res) => {
    const { classId } = req.params;
    ...
    try {
        // Verifica se o usuário é o dono da turma para editar
        const [rows] = await pool.query('SELECT owner_id FROM classes WHERE id = ?', [classId]);
        if (rows.length === 0) {
...
// --- API: Editar Dados do Aluno ---
app.post('/api/students/:studentId/edit', requireLogin, async (req, res) => {
    const { studentId } = req.params;
    // Assume que qualquer professor logado pode editar dados de um aluno (globalmente)
    // Se apenas membros da turma puderem, a lógica precisa ser mais complexa.
    const { fullName, cpf, pc_id } = req.body;

...
router.post('/logs', async (req, res) => {
    ...
    try {
        ...
        const values = await Promise.all(logs.map(async log => {
            ...
            // B. Existe regra óbvia de padrão? (Regex Rápido - AGORA INCLUI IA)
            else {
                const fastCat = fastCategorization(log.url);
                if (fastCat) {
                    category = fastCat;
                }
                // C. Se não sabe, pergunta para a IA Python (Último Recurso)
                else if (log.url) {
                    try {
                        category = await classifier.categorizar(log.url);
                    } catch (classifierError) {
                        console.error(`Erro ao classificar URL ${log.url}:`, classifierError);
                        // Em caso de erro da IA, mantém 'Não Categorizado' ou tenta deduzir
                        category = 'Outros'; 
                    }
                }
            }
